...
if ( !class_exists( 'SimpleAdsManagerAdmin' && class_exists('SimpleAdsManager') ) ) {
  class SimpleAdsManagerAdmin extends SimpleAdsManager {
    ...
    public function regAdminPage() {
	  ...
	  $this->editAds = add_submenu_page('sam-list', __('Ads Manager Editor', SAM_DOMAIN), __('Ads Manager', SAM_DOMAIN), SAM_ACCESS, 'sam-manager-edit', array(&$this, 'samManagerEditPage'));
      add_action('admin_print_styles-'.$this->editAds, array(&$this, 'adminEditStyles'));
	  add_action('admin_print_scripts-'.$this->editAds, array(&$this, 'adminEditScripts'));
      ...
      if(version_compare($wp_version, '3.3', '>=')) {
		...
		add_action('load-'.$this->editAds, array(&$this, 'samHelp'));
      }
		}

    public function samHelp() {
      $samScreens = array($this->listPage, $this->editPage, $this->listZone, $this->editZone, $this->listBlock, $this->editBlock, $this->settingsPage, $this->editAds);
      $samPages = array(
	...
	public function samManagerEditPage() {
      include_once('manager.editor.admin.class.php');
      $settings = parent::getSettings();
      $editor = new SamManagerEditor($settings);
      $editor->page();
    }
