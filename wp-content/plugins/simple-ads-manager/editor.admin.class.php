  ...
  class SamPlaceEdit {
    ...
    */
	...
	private function username($name) {
			global $wpdb;
          	$userName = $wpdb->get_results("SELECT ID, user_nicename FROM $wpdb->users");
   		  ?>
   		  <select name='ads_user' id='ads_user'>
           <?php
				foreach ($userName as $name1) 
					{
           ?>
           			<option value="<?php echo $name1->ID?>" <?php selected($name,$name1->user_nicename);?> ><?php echo $name1->user_nicename?></option>
           <?php
					}
           ?>
           </select>
        ...
        case 'item':
          ...
          if(isset($_POST['update_item'])) {
            ...
            $updateRow = array(
              ...
              'limit_clicks' => (isset($_POST['limit_clicks']) ? $_POST['limit_hits'] : 0),
              ...
              //FIXED 'trash' => ($_POST['trash'] === 'true')
              'trash' => ($_POST['trash'] === 'true' ? 1 : 0),
			  //set username id for the advertisement
			  'username_id'=>$_POST['ads_user']
            );
            ...
            $formatRow = array( '%d', '%s', '%s', '%d', '%d', '%s', '%s', '%s', '%d', '%s', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%s', '%d', '%d', '%s', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%d', '%d');
    ...
    <div class="metabox-holder has-right-sidebar" id="poststuff">
      <div id="side-info-column" class="inner-sidebar">
        <div class="meta-box-sortables ui-sortable">
          ...
          <!--change here-->
          <?php 
          $user_name = $wpdb->get_var( $wpdb->prepare("SELECT user_nicename FROM $wpdb->users WHERE ID =".$row['username_id' ]));
          			
          ?>
          <div id="submitdiv" class="postbox ">
            <div class="handlediv" title="<?php _e('Click to toggle', SAM_DOMAIN); ?>"><br/></div>
            <h3 class="hndle"><span><?php _e('Set User for Advertisement', SAM_DOMAIN);?></span></h3>
            <div class="inside">
               <div id="submitpost" class="submitbox">
                	<div id="misc-publishing-actions">
                    	<div class="misc-pub-section">
                    	<p><?php _e('Select user', SAM_DOMAIN);?></p>
            			<p>
                  		   <?php $this->username($user_name) ?>
                		</p>                		
						</div>
             		</div>
             	</div>
            ...
            <div id="previewdiv" class="postbox ">
              ...
              <div class="inside">
                <div class='ad-example'>
                  <?php 
                    $sample = new SamAd(array('id' => (integer) $row['id']), true, false);
                    echo $sample->ad; 
                  ?>
                </div>
              </div>
